...
len_number = len("number") # calculate only once outside the while loop
nbr = 0
while nbr < len(count_list):
    spaces = (len_number - len(str(nbr))) * " "
    print spaces + str(nbr) + ' | ' + str(count_list[nbr])
    nbr += 1
...
while nbr < len(count_list):
    spaces = (len_number - len(str(nbr))) * " "
    print spaces + str(nbr) + ' | ' + count_list[nbr] * "*"
    nbr += 1
